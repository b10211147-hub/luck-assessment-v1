...
async function load(q=""){
  ...
  $("#orders").innerHTML=rows.length?rows.map(o=>{const enterprise=o.activityTitle?.includes("老闆方案")||o.activityTitle?.includes("企業祈福");return `<article><header><b>${esc(o.name)}</b><span>${esc(o.id)}</span></header><dl><div><dt>LINE 名稱</dt><dd>${esc(o.lineDisplayName)||"—"}</dd></div><div><dt>來源官方</dt><dd>${esc(o.sourceName)||"歷史／未記錄"}</dd></div><div><dt>活動</dt><dd>${esc(o.activityTitle)}</dd></div><div><dt>${enterprise?"創立日期（民國）":"生日（民國）"}</dt><dd>${esc(formatBirthday(o.birthday))}</dd></div><div><dt>${enterprise?"公司地址（營業地址）":"地址"}</dt><dd>${esc(o.address)}</dd></div><div><dt>${enterprise?"公司人數":"人數"}</dt><dd>${esc(o.people)} 人</dd></div><div><dt>${enterprise?"祈福公司行號名":"祈福姓名"}</dt><dd>${esc(o.prayerNames)||"—"}</dd></div><div><dt>備註</dt><dd>${esc(o.note)||"—"}</dd></div><div><dt>狀態</dt><dd>${esc(o.status)}／${esc(o.paymentStatus)}</dd></div><div><dt>報名時間</dt><dd>${new Date(o.createdAt).toLocaleString("zh-TW")}</dd></div></dl></article>`}).join(""):"<p class=empty>目前沒有符合的報名資料</p>";
}
