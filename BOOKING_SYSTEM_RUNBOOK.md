# 奉母宮 LINE 預約系統施作基準

> 狀態：正式維護文件  
> 最後更新：2026-08-03  
> 適用範圍：奉母宮主官方、`@764catfn`、客人預約頁、LINE LIFF、預約 API、管理後台與 Telegram 通知。

## 1. 修改前必讀

所有預約系統相關修改都必須先閱讀本文件。修改登入、網址、來源參數、部署方式或 LINE 驗證流程時，必須同步更新本文件並完成第 7 節的測試。

## 2. 正式架構

| 項目 | 正式設定 |
|---|---|
| LIFF ID | `2010747679-nNL4BQhG` |
| LINE Login Channel ID | `2010747679` |
| LIFF Endpoint URL | `https://b10211147-hub.github.io/luck-assessment-v1/` |
| 預約前端 | `https://b10211147-hub.github.io/luck-assessment-v1/booking/` |
| 預約 API | `https://fengmugong-registration-api.b10211147.chatgpt.site` |
| 舊入口／轉址入口 | `https://fengmugong-registration-api.b10211147.chatgpt.site/booking` |
| 主官方 Telegram | 使用正式環境的 `TELEGRAM_CHAT_ID` |
| `@764catfn` Telegram | `@wendy138668`（與主官方共用同一個 Bot Token） |

LINE Login Channel ID、LIFF ID 與 Messaging API Channel ID 是不同用途，不可混用。前端使用完整 LIFF ID；後端驗證 Token 時使用 LINE Login Channel ID。

## 3. 兩個官方帳號的正式連結

### 奉母宮主官方 `@248ysvwi`

主要連結：

`https://liff.line.me/2010747679-nNL4BQhG/booking/?via=liff&src=main&flow=20260803b`

相容舊連結：

`https://fengmugong-registration-api.b10211147.chatgpt.site/booking?src=main`

### 第二官方 `@764catfn`

主要連結：

`https://liff.line.me/2010747679-nNL4BQhG/booking/?via=liff&src=764catfn&flow=20260803b`

相容舊連結：

`https://fengmugong-registration-api.b10211147.chatgpt.site/booking?src=764catfn`

官方帳號的圖文選單、歡迎訊息、QR Code 與人工傳送連結，優先使用主要 LIFF 連結。相容舊連結只用於既有連結無法立即更換時。

## 4. 2026-08-03 LINE 閃退／反覆跳轉事件

### 使用者症狀

- 手機 LINE 開啟後畫面閃退或反覆重開。
- 停留在「正在連接 LINE 身分」。
- 一般網址、GitHub Pages Endpoint 與 LINE 登入頁之間重複跳轉。
- 部分環境曾出現 `400 Bad Request`。

### 根本原因

客人進入帶有子路徑的 LIFF 網址後，LINE 會先把使用者送到 Endpoint 根目錄，並以 `liff.state` 保存 `/booking/` 與來源參數。舊版根頁面在 `liff.init()` 完成前自行呼叫 `window.location.replace()`，跳過 LINE 規定的 primary redirect 初始化，也可能遺失登入狀態。這會讓手機 LINE 在 Endpoint、登入頁與預約頁之間重複開啟，表現成閃退或反覆跳轉。

### 已採用修正

1. 所有正式 LIFF 入口必須帶上 `via=liff`。
2. Endpoint 根頁面收到 `liff.state` 時，不可在 `liff.init()` 完成前以 `location`、History API 或伺服器 301／302 改變網址。
3. 根頁面必須先執行 `liff.init()`，由 LIFF SDK 將 primary redirect 正常轉成 `/booking/` 的 secondary redirect。
4. `/booking/` secondary redirect 必須再次執行 `liff.init()`；完成後若仍未登入，才使用 `liff.login({ redirectUri: window.location.href })`。
5. GitHub Pages 預約頁若沒有 `via=liff`，只保留所有參數並轉到 LIFF 官方入口一次。
6. Sites 的舊入口必須合併原查詢參數、強制加入 `via=liff`，再轉到 LIFF 官方入口。
7. `src` 必須在全部轉址中保留，避免主官方與 `@764catfn` 來源混淆。

## 5. 不可破壞的實作規則

1. 不可把 GitHub Pages 原始網址當作官方帳號的主要入口。
2. 不可移除 `via=liff`，也不可用 `target.search = ...` 覆蓋既有 LIFF 必要參數；應逐項合併參數。
3. Endpoint 根頁面出現 `liff.state` 時，必須先等待 `liff.init()`；初始化完成前嚴禁自行改寫或轉址。
4. `src=main` 代表奉母宮主官方；`src=764catfn` 代表第二官方。
5. LINE 身分以後端驗證為準，不信任前端自行傳入的 LINE 名稱或 User ID。
6. 身分驗證以 ID Token 為主，Access Token Profile 驗證為備援；兩者皆失敗才回傳 401。
7. LINE API 網路或 JSON 解析錯誤必須捕捉，不能讓 `/api/identity` 產生空白 500。
8. LIFF Endpoint 必須維持在專案根路徑；子路徑由 LIFF SDK 依 `liff.state` 導回，不能在初始化前自行拼接或改寫 `/booking/`。
9. 發布前端 JavaScript 後必須更新資源版本參數，避免手機沿用舊快取。
10. 修正 LIFF 啟動流程後要更新正式入口的 `flow` 版本參數，強制手機避開舊的 GitHub Pages／LIFF 快取。
11. 舊入口要維持可用，但正式宣傳一律使用第 3 節的主要 LIFF 連結。

## 6. 主官方與第二官方的隔離

- 兩個官方可共用同一個預約前端與 API，但必須用 `src` 分流。
- Telegram 與管理後台必須顯示正確來源官方。
- `@764catfn` 的 Telegram 報名與預約通知固定送到 `@wendy138668`；主官方仍使用 `TELEGRAM_CHAT_ID`，兩者共用 `TELEGRAM_BOT_TOKEN`。
- `@764catfn` 的品牌、服務項目、預約須知與後台權限可獨立調整，不得改壞主官方畫面。
- 未帶 `src` 時一律視為 `main`，確保主官方的舊連結相容。

## 7. 每次發布後的最低測試

### 主官方

- 開啟主官方主要 LIFF 連結。
- 確認沒有 400、沒有反覆跳轉、沒有停在連接畫面。
- 確認 LINE 暱稱成功取得。
- 確認來源為 `main`／`@248ysvwi`。

### `@764catfn`

- 開啟 `@764catfn` 主要 LIFF 連結。
- 確認顯示 `@764catfn` 專屬品牌與內容。
- 確認 LINE 暱稱成功取得。
- 確認 Telegram 與後台來源為 `@764catfn`。
- 確認 Telegram 通知送到 `@wendy138668`，未誤送到主官方頻道。

### 共通

- LINE App 內測試一次。
- 手機外部瀏覽器測試一次。
- 舊 Sites 入口測試一次，確認只轉址一次。
- GitHub Pages 直連測試一次，確認會轉到 LIFF 入口。
- 檢查 `/api/identity`、時段載入與送出預約。
- 確認網址中的 `via=liff` 與 `src` 在登入後仍存在。

## 8. 發布順序

1. 修改前先閱讀本文件並確認不破壞第 5 節規則。
2. 先完成前端與後端建置檢查。
3. 發布 GitHub Pages 預約前端。
4. 若改到 API 或舊入口，再發布 Sites 後端。
5. 等待 GitHub Pages 與 Sites 正式版本生效。
6. 依第 7 節完成兩個官方帳號的測試。
7. 若本次問題產生新規則，先更新本文件再結案。

## 9. 預約取消管理

- 主官方與 `@764catfn` 後台皆有「取消管理」頁籤，只列出尚未到期且狀態為已確認的案件。
- 「取消預約」會把案件標記為 `cancelled`，保留完整歷史紀錄，原時段改為 `closed`，不提供其他客人預約。
- 「取消並重新上架」會把案件標記為 `cancelled`，原時段改回 `open`，讓其他客人可以再次預約。
- 已取消案件不可重複取消；已過期時段不可重新上架。
- `appointments.slot_id` 只限制同一時段最多有一筆 `confirmed` 案件；取消後重新預約會建立新案件，舊紀錄不得刪除或覆蓋。
- 每次發布後必須測試兩種取消方式，並確認主官方與 `@764catfn` 的資料仍依 `source_code` 隔離。

## 10. 時段批次刪除

- 主官方與 `@764catfn` 的「近期時段」皆可勾選多個未預約時段，再以單次請求批次刪除，避免逐筆操作延遲。
- 「全選可刪除時段」只選取 `open` 或 `closed` 狀態，不得選取或刪除 `booked` 時段。
- 「一鍵刪除全部」只刪除該來源官方尚未到期且未被預約的時段，不影響另一個官方，也不影響已預約案件。
- 單筆、批次與全部刪除都必須先顯示確認方塊；使用者取消確認時不可送出刪除請求。
- 批次刪除一次最多接受 500 個時段 ID，後端必須再次檢查 `source_code`、未到期與 `status != 'booked'`。

## 11. 行事曆與內部行程

- 客人端以月曆顯示日期，只有存在可預約時段或公開行程的日期可點選；選定日期後才顯示當日可預約時間。
- 後台月曆以顏色區分可預約、已預約、已關閉與內部行程，點選日期後只管理該日明細。
- 內部行程分為「私人」與「公開」：私人行程名稱與備註只可由已登入後台讀取；公開行程名稱可顯示在客人月曆。
- 開啟「這段時間不可被客人預約」時，與行程重疊的開放時段不得出現在客人端；客人送出預約時，後端必須再次檢查重疊，避免同時操作造成衝突。
- 阻擋行程不得覆蓋既有 `confirmed` 預約；若時間內已有客人預約，新增或修改行程必須回傳衝突訊息。
- 行程、時段與預約都必須依 `source_code` 隔離；`main` 與 `764catfn` 的月曆不可互相顯示資料。
- 客人端只可匿名讀取 `visibility = 'public'` 的行程；完整行程 CRUD 一律要求該來源官方的管理密碼。
- 內部行程可設為「全天行程」；後台只輸入一個日期，資料以該日台北時間 00:00 至隔日 00:00 保存，公開行程在客人端顯示「全天」。
- 本日批次勾選只作用於月曆選取日；「一鍵刪除全部」仍只刪除該來源官方全部未來且未被預約的時段。
- 發布後至少測試：公開／私人行程顯示、阻擋與不阻擋時段、既有預約衝突、兩個官方資料隔離、月曆月份切換與手機版操作。

## 12. 後台代客預約

- 管理者可在後台月曆選定日期後，從該日尚未被預約且未受內部行程阻擋的時段中選擇一筆，手動輸入客人姓名與備註建立預約。
- 代客預約必須通過該來源官方的管理密碼驗證，不要求客人登入 LINE，也不得信任沒有管理授權的 `manual` 請求。
- 代客預約在資料庫與 Telegram 通知中標示為「後台代客預約」，LINE 名稱顯示未連結，LINE User ID 不對外顯示。
- 後台代客預約與客人 LINE 預約共用同一套時段占用、內部行程衝突、取消及重新上架規則。
- 主官方與 `@764catfn` 的代客預約資料及 Telegram 通知仍須依 `source_code` 完整隔離。

## 13. 客人端預約已滿顯示

- 客人端月曆必須取得不含個資的每日預約數量摘要；不得把客人姓名、LINE 資料或預約案件內容傳到公開 API。
- 某日所有尚未到期且原本可供預約的時段皆為 `booked` 時，月曆顯示「預約已滿」，該日期仍可點選查看提示。
- 點選已滿日期後，可列出該日時間並標示「已被預約」，但按鈕必須停用，且不得顯示任何客人資料；若沒有可列出的時間才顯示「本日預約已滿，請選擇其他日期」。
- 只有 `closed` 或受內部行程阻擋、但沒有全部被預約的日期，不得誤標為「預約已滿」。
- 每日摘要仍須依 `source_code` 隔離，主官方與 `@764catfn` 不得互相計入時段。

## 14. 客人端八小時預約截止

- 客人端距離時段開始時間剩 8 小時（含）以內時，該時段顯示「已被預約」並停用，不得再讓客人選取。
- 公開每日摘要計算時，8 小時內仍為 `open` 的時段視同客人端不可預約；若該日所有剩餘時段皆為已預約或已進入截止時間，月曆顯示「預約已滿」。
- 客人送出預約時，後端必須再次比較伺服器時間；8 小時內的非後台預約一律回傳衝突，避免舊快取或直接呼叫 API 繞過限制。
- 管理密碼驗證通過的「後台代客預約」不受八小時截止限制，讓業主可以處理臨時案件。
- 八小時規則只影響客人端可預約性，不得把資料庫中的 `open` 時段永久改成 `booked`，也不得建立假的預約案件。
